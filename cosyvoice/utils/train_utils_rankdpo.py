...
def batch_forward(model, batch, scaler, info_dict, ref_model=None, rankdpo_loss=None):
    ...
    with autocast:
        ...
        if rankdpo_loss:
            logps = info_dict['loss_dict']["logps"]
            rewards = batch["rewards"].to(device)
            sft_loss = info_dict['loss_dict']['loss']
            
            if rankdpo_loss.rf_free:
                preference_loss = rankdpo_loss(logps, rewards)
            else:
                with torch.no_grad():
                    ref_model = ref_model.to(device)
                    ref_loss_dict = ref_model(batch, device)
                ref_logps = ref_loss_dict["logps"]
                preference_loss = rankdpo_loss(logps, rewards, ref_logps)
                
            sorted_idx_by_logp = logps.argsort(descending=True)
            sorted_idx_by_reward = rewards.argsort(descending=True)
            rank_acc = (sorted_idx_by_logp == sorted_idx_by_reward).float().mean()
            # 位置命中率
            ...
            for i in range(k):
                for j in range(i+1, k):
                    same = ((logps[:, i] - logps[:, j]) * (rewards[:, i] - rewards[:, j]) > 0)
                    pairs.append(same.float())
            pair_acc = torch.stack(pairs, 0).mean(0).mean()
            ...
            info_dict['loss_dict']["rank_acc"] = rank_acc
            info_dict['loss_dict']["pair_acc"] = pair_acc
            info_dict['loss_dict']["logps"] = logps
            info_dict['loss_dict']["rewards"] = rewards
