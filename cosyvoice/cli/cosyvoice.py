...
class CosyVoice2(CosyVoice):
    ...
    def __init__(self, model_dir, load_jit=False, load_trt=False, load_vllm=False, fp16=False, trt_concurrent=1):
        ...
        self.frontend = CosyVoiceFrontEnd(configs['get_tokenizer'],
                                          ...
                                          '{}/pre_models/spk2info.pt'.format(model_dir),
                                          configs['allowed_special'])
        self.sample_rate = configs['sample_rate']
        if torch.cuda.is_available() is False and (load_jit is True or load_trt is True or fp16 is True):
            load_jit, load_trt, fp16 = False, False, False
    ...
    def inference_instruct_spk(self, tts_text, spk_id, stream=False, speed=1.0, text_frontend=True):
        instruct_id = self.frontend.text_normalize(spk_id, split=False, text_frontend=text_frontend)
        for i in tqdm(self.frontend.text_normalize(tts_text, split=True, text_frontend=text_frontend)):
            model_input = self.frontend.frontend_instruct_spk(i, spk_id, instruct_id)
            start_time = time.time()
            logging.info('synthesis text {}'.format(i))
            for model_output in self.model.tts(**model_input, stream=stream, speed=speed):
                speech_len = model_output['tts_speech'].shape[1] / self.sample_rate
    ...
    def inference_zero_shot_cross(self, tts_text,prompt_speech_16k, spk_id='', stream=False, speed=1.0, text_frontend=True):
        # prompt_text = self.frontend.text_normalize(prompt_text, split=False, text_frontend=text_frontend)
        for i in tqdm(self.frontend.text_normalize(tts_text, split=True, text_frontend=text_frontend)):
            # if (not isinstance(i, Generator)) and len(i) < 0.5 * len(prompt_text):
            #     logging.warning('synthesis text {} too short than prompt text {}, this may lead to bad performance'.format(i, prompt_text))
            model_input = self.frontend.inference_zero_shot_cross(i,  prompt_speech_16k, self.sample_rate, spk_id)
            start_time = time.time()
            logging.info('synthesis text {}'.format(i))
            for model_output in self.model.tts(**model_input, stream=stream, speed=speed):
                speech_len = model_output['tts_speech'].shape[1] / self.sample_rate
