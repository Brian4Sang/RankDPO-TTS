...
class Qwen2LM(TransformerLM):
    def __init__(
        ...
        self.llm_embedding = torch.nn.Embedding(2, llm_input_size)
        self.llm = llm
        self.llm.model.config.output_hidden_states = True
        ...
        # 5. [Optional] set rankdpo
        self.rankdpo = rankdpo
    ...
    def forward(
        ...
        speech_token_len = batch['speech_token_len'].to(device)
        if self.rankdpo:
            mid_speech_token = batch['mid_speech_token'].to(device)
            mid_speech_token_len = batch['mid_speech_token_len'].to(device)
            rej_speech_token = batch['rej_speech_token'].to(device)
            rej_speech_token_len = batch['rej_speech_token_len'].to(device)
        # 1. prepare llm_target
        sos_eos_emb = self.llm_embedding.weight[self.sos_eos].reshape(1, 1, -1)
        task_id_emb = self.llm_embedding.weight[self.task_id].reshape(1, 1, -1)
        target_ids = [torch.tensor([IGNORE_ID] * (1 + text_token_len[i]) + speech_token[i, :speech_token_len[i]].tolist() +
                                        [self.speech_token_size]) for i in range(text_token.size(0))]
        if self.rankdpo:
            mid_target_ids = [torch.tensor([IGNORE_ID] * (1 + text_token_len[i]) + mid_speech_token[i, :mid_speech_token_len[i]].tolist() +
                                            [self.speech_token_size]) for i in range(text_token.size(0))]
            target_ids.extend(mid_target_ids)
            rej_target_ids = [torch.tensor([IGNORE_ID] * (1 + text_token_len[i]) + rej_speech_token[i, :rej_speech_token_len[i]].tolist() +
                                            [self.speech_token_size]) for i in range(text_token.size(0))]
            target_ids.extend(rej_target_ids)
        target_ids = pad_sequence(target_ids, batch_first=True, padding_value=IGNORE_ID).to(device) ###[B*3, T]

        # 2. speech token projection
        speech_emb = self.speech_embedding(speech_token)
        if self.rankdpo:
            mid_speech_emb = self.speech_embedding(mid_speech_token)
            rej_speech_emb = self.speech_embedding(rej_speech_token)
        ...
        if self.rankdpo:
            mid_speech_emb = unpad_sequence(mid_speech_emb, mid_speech_token_len.cpu(), batch_first=True)
            mid_input_emb = [torch.concat([sos_eos_emb.squeeze(dim=0), text_emb[i], task_id_emb.squeeze(dim=0), mid_speech_emb[i]], dim=0)
                                for i in range(len(text_emb))]
            input_emb.extend(mid_input_emb)
            
            rej_speech_emb = unpad_sequence(rej_speech_emb, rej_speech_token_len.cpu(), batch_first=True)
            rej_input_emb = [torch.concat([sos_eos_emb.squeeze(dim=0), text_emb[i], task_id_emb.squeeze(dim=0), rej_speech_emb[i]], dim=0)
                                for i in range(len(text_emb))]
            input_emb.extend(rej_input_emb)
        input_emb_lengths = torch.tensor([i.size(0) for i in input_emb], dtype=torch.int32).to(device)
        input_emb = pad_sequence(input_emb, batch_first=True, padding_value=IGNORE_ID).to(device) ###[B*3, T, D]
        ...
        if not self.rankdpo:
            ...
            logps = all_logps_mean.view(text_token.size(0),3)
            return {
                "loss": loss,
                "acc": acc,
                "logps": logps,
